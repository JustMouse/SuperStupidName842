...
var/list/global_sidormat_list = list(
	...
	"Автоматическое оружие и ПП" = list(
		///////////////////////////// **Автоматы, ПП** /////////////////////////////////////
	...
	"Верхняя одежда" = list(
		...
		new	/datum/data/stalker_equipment("Exoskeleton",				"Экзоскелет",						/obj/item/clothing/suit/hooded/sealed/exoskelet,			150000,	EXPERT,)
		),
	...
	for(var/atom/movable/I in ontable)
		...
		if(istype(I, /obj/item/ammo_box))
			...
			if(AB.stored_ammo.len < AB.max_ammo)
				say("[I] был продан за [GetCost(I)/2].")
		else
	...
	for(var/atom/movable/AM in itemloc)
		...
		if(istype(AM, /obj/item/clothing))
			...
			if((C.durability / initial(C.durability)) * 100 < 80)
				say("[AM] слишком испорчен(а) для продажи")
				continue
